...
def parse_args_paired_testing(input_args=None):
   ...
   """
    ...
    parser.add_argument("--base_config", default="./configs/test.yaml", type=str)
...
class SingleDataset(data.Dataset):
    """Dataset for loading single-channel (e.g. infrared) images,
    but returns a 3-channel tensor for compatibility with RGB-based models.

    Args:
        opt (dict): Config for dataset. It contains the following keys:
            lq_path (str or list): Path(s) to images.
            io_backend (dict): IO backend type and other kwarg.
            image_type (str): Image extension (default: 'png').
    """
    ...
    def __init__(self, opt):
        ...
        if 'lq_path' in opt:
            if isinstance(opt['lq_path'], str):
                self.lq_paths.extend(sorted(
                    [str(x) for x in Path(opt['lq_path']).glob('*.png')] +
                    [str(x) for x in Path(opt['lq_path']).glob('*.jpg')] +
                    [str(x) for x in Path(opt['lq_path']).glob('*.jpeg')]
                ))
            else:
                for path in opt['lq_path']:
                    self.lq_paths.extend(sorted([str(x) for x in Path(path).glob('*.' + opt['image_type'])]))
    ...
    def __getitem__(self, index):
        ...
        while retry > 0:
            try:
                lq_img_bytes = self.file_client.get(lq_path, 'gt')
        ...
        # Load single-channel image (gray=True)
        img_lq = imfrombytes(lq_img_bytes, float32=True, flag='grayscale')  # shape: (H, W)

        # Convert to 3-channel by repetition
        img_lq = img_lq[:, :, None].repeat(3, axis=2)  # (H, W, 3)
