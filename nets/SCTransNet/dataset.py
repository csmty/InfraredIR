from nets.SCTransNet.utils import *
...
class TestSetLoader(Dataset):
    def __init__(self, dataset_dir, train_dataset_name, test_dataset_name, gt_dir, img_norm_cfg=None):
        super(TestSetLoader).__init__()
        self.dataset_dir = dataset_dir
        self.gt_dir = gt_dir
        label_txt = os.path.join(gt_dir, 'img_idx', f'test_{test_dataset_name}.txt')
    ...
    def __getitem__(self, idx):
        mask = Image.open((self.gt_dir + '/masks_val/' + self.test_list[idx] + '.png').replace('//', '/'))
        try:
            # img = Image.open((self.dataset_dir + '/imgs/' + self.test_list[idx] + '.png').replace('//', '/')).convert('I')
            img = Image.open((self.dataset_dir + '/' + self.test_list[idx] + '.png').replace('//', '/')).convert('I')
        except:
            # img = Image.open((self.dataset_dir + '/imgs/' + self.test_list[idx] + '.jpg').replace('//', '/')).convert('I')
            img = Image.open((self.dataset_dir + '/' + self.test_list[idx] + '.jpg').replace('//', '/')).convert('I')
