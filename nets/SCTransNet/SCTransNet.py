...
class SCTransNet(nn.Module):
    def __init__(self, config, n_channels=1, n_classes=1, img_size=256, vis=False, mode='train', deepsuper=True):
        ...
        if self.deepsuper:
            ...
            # self.gt_conv5 = nn.Sequential(nn.Conv2d(in_channels * 8, 2, 1))
            # self.gt_conv4 = nn.Sequential(nn.Conv2d(in_channels * 4, 2, 1))
            # self.gt_conv3 = nn.Sequential(nn.Conv2d(in_channels * 2, 2, 1))
            # self.gt_conv2 = nn.Sequential(nn.Conv2d(in_channels * 1, 2, 1))
            # self.outconv = nn.Conv2d(5 * 2, 2, 1)
