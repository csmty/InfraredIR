...
import threading
from nets.SCTransNet.dataset import *
import time
from collections import OrderedDict
from nets.SCTransNet.SCTransNet import SCTransNet as SCTransNet
# from loss import *
import nets.SCTransNet.Config as config
import numpy as np
import torch
...
parser.add_argument("--model_names", default='SCTrans', type=str, help="Model name, e.g. 'SCTrans'")
parser.add_argument("--weight_pth", required=True, type=str)
parser.add_argument("--dataset_dir", required=True, type=str, help="test_dataset_dir")
parser.add_argument("--gt_dir", required=True, type=str, help="gt_dataset_dir")
parser.add_argument("--dataset_names", required=True, type=str,
                    help="dataset_name: 'NUAA-SIRST', 'NUDT-SIRST', 'IRSTD-1K', 'SIRST3', 'NUDT-SIRST-Sea'")
parser.add_argument("--img_norm_cfg", default=None, type=dict,
                    help="specific a img_norm_cfg, default=None (using img_norm_cfg values of each dataset)")
parser.add_argument("--save_img", default=True, type=bool, help="save image of or not")
parser.add_argument("--save_img_dir", required=True, type=str,
                    help="path of saved image")
parser.add_argument("--save_log", required=True, type=str, help="path of saved .pth")
...
def test():
    test_set = TestSetLoader(opt.dataset_dir, opt.train_dataset_name, opt.test_dataset_name, opt.gt_dir, opt.img_norm_cfg)
    ...
    with torch.no_grad():
        ...
        save_txt_path = os.path.join(opt.save_img_dir, "metrics_results.txt")
        os.makedirs(opt.save_img_dir, exist_ok=True)

        with open(save_txt_path, "w") as f:
            f.write(f"测试结果\n")
            f.write("=" * 50 + "\n\n")
            f.write(f"pixAcc (×100): {pixAcc * 100:.4f}\n")
            f.write(f"mIoU (×100): {mIOU * 100:.4f}\n")
            f.write(f"nIoU (×100): {nIoU * 100:.4f}\n")
            f.write(f"Pd (×100): {results2[0] * 100:.4f}\n")
            f.write(f"Fa (×1e6): {results2[1] * 1e6:.4f}\n")
            f.write(f"F1-score (×100): {F1_score * 100:.4f}\n")
...
if __name__ == '__main__':
    ...
    log_path = os.path.join(
        opt.save_log,
        'test_' + time.ctime().replace(' ', '_').replace(':', '_') + '.txt'
    )
    opt.f = open(log_path, 'w')

    # Assign single-value arguments directly.
    ...
    # Use the full checkpoint path directly.
    opt.pth_dir = opt.weight_pth

    # Print and save run information.
